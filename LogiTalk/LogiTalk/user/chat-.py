...
def auto_size():
    ...
    if menu_is_open:
        ...
        online_Textbox.delete('1.0', 'end')
        online_Textbox.insert('end', online)
        online_Textbox.pack(fill='both')
...
def rswmsg():
    try:
        ...
        print(msg)
        print(msg[0:len('new_online_')])
        if msg[0:len('new_online_')] == 'new_online_':
            global online
            online = ast.literal_eval(msg[len('new_online_')::])
            print(online)
        else:
...
online_Textbox = customtkinter.CTkTextbox(frame1, fg_color='#ffffff', state='normal', font=(None, 15))
